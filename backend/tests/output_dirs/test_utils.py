...
class TestValidateRoot:
    async def test_creates_root_when_missing(self, tmp_path: Path) -> None:
        target = tmp_path / "new-root"
        assert not target.exists()
        resolved = await validate_root(str(target))
        assert resolved == target.resolve()
        assert target.is_dir()

    async def test_accepts_existing_dir(self, tmp_path: Path) -> None:
        existing = tmp_path / "media"
        existing.mkdir()
        resolved = await validate_root(str(existing))
        assert resolved == existing.resolve()

    async def test_rejects_relative_path(self) -> None:
        with pytest.raises(HTTPException) as exc_info:
            await validate_root("relative/path")
        assert exc_info.value.status_code == 400
        assert "absolute" in exc_info.value.detail

    async def test_rejects_when_parent_missing(self, tmp_path: Path) -> None:
        missing_parent = tmp_path / "no-such-dir" / "media"
        with pytest.raises(HTTPException) as exc_info:
            await validate_root(str(missing_parent))
        assert exc_info.value.status_code == 400
        assert "parent" in exc_info.value.detail

    async def test_rejects_when_mkdir_fails(self, tmp_path: Path) -> None:
        # Path component that exists as a regular file blocks mkdir.
        collider = tmp_path / "file"
        collider.write_bytes(b"")
        with pytest.raises(HTTPException) as exc_info:
            await validate_root(str(collider / "would-be-root"))
        assert exc_info.value.status_code == 400
        assert "cannot create root" in exc_info.value.detail

    async def test_rejects_when_not_writable(self, tmp_path: Path, monkeypatch) -> None:
        ...
        with pytest.raises(HTTPException) as exc_info:
            await validate_root(str(root))
        assert exc_info.value.status_code == 400
        assert "not writable" in exc_info.value.detail


class TestValidateUnderRoot:
    async def test_accepts_path_under_root(self, tmp_path: Path) -> None:
        root = tmp_path / "media"
        root.mkdir()
        out = root / "Manga"
        resolved = await validate_under_root(str(out), root)
        assert resolved == out.resolve()
        assert out.is_dir()

    async def test_accepts_root_itself(self, tmp_path: Path) -> None:
        root = tmp_path / "media"
        root.mkdir()
        # The router uses this when postprocess_default_output_dir == root.
        resolved = await validate_under_root(str(root), root)
        assert resolved == root.resolve()

    async def test_rejects_relative_path(self, tmp_path: Path) -> None:
        root = tmp_path / "media"
        root.mkdir()
        with pytest.raises(HTTPException) as exc_info:
            await validate_under_root("relative", root)
        assert exc_info.value.status_code == 400
        assert "absolute" in exc_info.value.detail
        assert "output_dir" in exc_info.value.detail

    async def test_uses_custom_field_in_error_message(self, tmp_path: Path) -> None:
        root = tmp_path / "media"
        root.mkdir()
        with pytest.raises(HTTPException) as exc_info:
            await validate_under_root("relative", root, field="default")
    ...
    async def test_rejects_path_outside_root(self, tmp_path: Path) -> None:
        ...
        outside.mkdir()
        with pytest.raises(HTTPException) as exc_info:
            await validate_under_root(str(outside), root)
        assert exc_info.value.status_code == 400
        assert "must be under root" in exc_info.value.detail
        assert str(root.resolve()) in exc_info.value.detail

    async def test_rejected_path_is_not_created(self, tmp_path: Path) -> None:
    ...
    async def test_rejects_when_mkdir_fails(self, tmp_path: Path) -> None:
        ...
        collider.write_bytes(b"")
        with pytest.raises(HTTPException) as exc_info:
            await validate_under_root(str(collider / "would-be-dir"), root)
        assert exc_info.value.status_code == 400
        assert "cannot create output_dir" in exc_info.value.detail

    async def test_skips_create_when_disabled(self, tmp_path: Path) -> None:
        ...
        result = await validate_under_root(str(missing), root, create=False)
    ...
    async def test_rejects_when_not_writable(self, tmp_path: Path, monkeypatch) -> None:
        ...
        monkeypatch.setattr(Path, "write_bytes", deny_writes)
        with pytest.raises(HTTPException) as exc_info:
            await validate_under_root(str(out), root)
        assert exc_info.value.status_code == 400
