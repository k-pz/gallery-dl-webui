...
from backend.poller import _parse_iso, is_due
...
async def test_list_watched_targets_returns_watched_only(storage: Storage) -> None:
    a = await storage.upsert_target("https://x/a", None, None)
    b = await storage.upsert_target("https://x/b", None, None)
    await storage.upsert_target("https://x/c", None, None)
    await storage.update_target(a.id, watched=True)
    await storage.update_target(b.id, watched=True)

    rows = await storage.list_watched_targets()
