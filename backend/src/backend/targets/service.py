...
async def set_series_status(
    db: aiosqlite.Connection, id_: int, status: str | None
) -> Target | None:
    """Persist the auto-detected series publication status (no-op when empty/None).

    Called by the download worker after the manifest simulation pass discovers
    a status value from gallery-dl.  The value has already been normalised to
    one of the Komga-compatible tokens (Ongoing / Ended / Hiatus / Abandoned)
    before this function is invoked.
    """
    if not status:
        return await get(db, id_)
    await db.execute("UPDATE targets SET series_status = ? WHERE id = ?", (status, id_))
    await db.commit()
    return await get(db, id_)
